...
function hit_sphere(center::Vec3, radius::Float64, ray::Ray)
    ...
    discriminant >= 0  # We'll fix negative t values later
end

function ray_color(ray::Ray)
    if hit_sphere(Vec3(0.0, 0.0, -1.0), 0.5, ray)
        return Vec3(1.0, 0.0, 0.0)  # Red for sphere hits
    end
    
    # Background gradient (keep from previous chapter)
...
function main()
    ...
    # Render to PPM
    open("images/ch5_image.ppm", "w") do file
        println(file, "P3")
